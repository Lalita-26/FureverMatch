    ...
    useEffect(() => {
      fetch("/breeds.json") // ระบุตำแหน่งของไฟล์ breeds.json ในโฟลเดอร์ public
        ...
        .then((data) => {
          ...
          const pomeranian = data.find((breed) => breed.name === "Pomeranian");
          setSelectedBreed(pomeranian); // กำหนดสายพันธุ์ที่เลือก
        })
          ...
          <div className="bg-white shadow-xl rounded-lg p-4 sm:p-6 max-w-4xl w-full ">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:space-x-6 border-b-2 border-gray-300 pb-4">
              ...
              <div className="mt-4 sm:mt-0 sm:ml-6 border-2 border-gray-200 p-4 rounded-lg flex-1">
                ...
                <p className="mb-2">
                  <strong>ประเภท:</strong> {selectedBreed.titletype}
                </p>
                <p className="mb-2">
                  <strong>แหล่งกำเนิด:</strong> {selectedBreed.origin}
                </p>
                <p className="mb-2">
                  <strong>อุปนิสัย:</strong>{" "}
                  {selectedBreed.temperament.join(", ")}
                </p>
                <p className="mb-2">
                  <strong>ขนาด:</strong> {selectedBreed.size}
                </p>
                <p className="mb-2">
                  <strong>อายุขัย:</strong> {selectedBreed.lifespan}
                </p>
                <p className="mb-2">
                  <strong>การมีปฏิสัมพันธ์กับคน:</strong>{" "}
                  {selectedBreed.interactionpeople}
                </p>
                <p className="mb-2">
                  <strong>การมีปฏิสัมพันธ์กับสัตว์:</strong>{" "}
                  {selectedBreed.interactionAnimals}
                </p>
                <p className="mb-2">
                  <strong>ปัญหาสุขภาพ:</strong>{" "}
                  {selectedBreed.healthIssues.join(", ")}
                </p>
              </div>
            </div>
